...
function replaceText(text, oldString, newString, replaceAll) {
...
function findReplacement(text, oldString, newString, newline) {
  if (text.includes(oldString)) {
    return { searchText: text, searchOld: oldString, searchNew: newString };
  }

  const adaptedOld = normalizeNewlines(oldString, newline);
  const adaptedNew = normalizeNewlines(newString, newline);
  if (text.includes(adaptedOld)) {
    return { searchText: text, searchOld: adaptedOld, searchNew: adaptedNew };
  }

  const unifiedText = toLf(text);
  const unifiedOld = toLf(oldString);
  const unifiedNew = toLf(newString);
  if (unifiedText.includes(unifiedOld)) {
    return { searchText: unifiedText, searchOld: unifiedOld, searchNew: unifiedNew };
  }
...
export function replaceInFile(filePath, oldString, newString, replaceAll = false) {
  ...
  const { updated, count } = replaceText(
    match.searchText,
    match.searchOld,
    match.searchNew,
    replaceAll
